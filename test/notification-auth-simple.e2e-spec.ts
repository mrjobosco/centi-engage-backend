...
describe('Notification Authentication & Authorization (e2e) - Simple Tests', () => {
  ...
  describe('Authorization Tests - User Access Control', () => {
    describe('Notification ownership', () => {
      ...
      it('should return 404 when trying to access non-existent notifications', async () => {
        await request(app.getHttpServer())
      ...
      it('should allow users to access their own notification preferences', async () => {
        ...
        // All preferences should belong to the authenticated user
  ...
  describe('Authorization Tests - Tenant Isolation', () => {
    describe('Cross-tenant access prevention', () => {
      ...
      it('should prevent users from accessing preferences with wrong tenant header', async () => {
        const response = await request(app.getHttpServer())
          .get('/notification-preferences')
          .set('x-tenant-id', tenant2.id)
          .set('Authorization', `Bearer ${user1Token}`);
  ...
  describe('Admin Role Tests', () => {
    it('should allow admin users to broadcast notifications to tenant', async () => {
      ...
      const response = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${user1Token}`)
        .send({
          userId: user1.id, // This might be ignored by the broadcast endpoint
          type: 'INFO',
          category: 'system',
