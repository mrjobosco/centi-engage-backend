...
describe('Notification Working Integration Tests', () => {
  ...
  describe('Channel Factory', () => {
    ...
    it('should get channel handlers from factory', () => {
      ...
      expect(inAppChannelFromFactory).toBe(inAppChannel);
      expect(emailChannelFromFactory).toBe(emailChannel);
      expect(smsChannelFromFactory).toBe(smsChannel);
    });
  });
  ...
  describe('Database Operations', () => {
    ...
    it('should handle multiple notifications for same user', async () => {
      ...
      // Note: Current implementation creates multiple notifications per service call
      // We should have notifications for both calls, but may be more than 2 due to channels
      expect(notifications.length).toBeGreaterThan(0);
      ...
      // Verify the main notifications exist
      const hasFirstMain = notifications.some((n) => n.id === notification1.id);
      const hasSecondMain = notifications.some(
        (n) => n.id === notification2.id,
      );

      expect(hasFirstMain).toBe(true);
      expect(hasSecondMain).toBe(true);
    });
  });
});
